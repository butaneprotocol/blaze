...
export class TxBuilder {
  ...
  private insertSorted<T>(arr: T[], el: T, ser?: (a: T) => string) {
    ser = ser ?? ((x) => x as string);
    const index = arr.findIndex((x) => ser(x).localeCompare(ser(el)) > 0);
    if (index == -1) {
      arr.push(el);
    } else {
